...
- (void)WXPayPostDic:(NSDictionary *)dic  Debug:(void(^)(NSString *error))errorBlock CallWXpay:(void(^)(NSString *wrong))callWXpay {
    ...
    if(dic == nil){
        ...
        req.openID              = [NSString stringWithFormat:@"%@",[dic objectForKey:@"appid"]];
...
- (void)payWithPayMethod:(NSInteger )payMethod
        ...
        NSString *str = [NSString stringWithFormat:@"%@?orderNo=%@&payMethod=%@&couponId=%@",buyClothPay_Url,orderNo,@(payMethod),@"0"];
