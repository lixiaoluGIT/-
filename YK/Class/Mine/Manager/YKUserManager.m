...
- (void)loginSuccessByTencentDic:(NSDictionary *)Dic OnResponse:(void (^)(NSDictionary *dic))onResponse{
    ...
    if ([Token length] != 0) {//已登录，绑定qq
       
        [self bindingTencentWithDictionary:dic OnResponse:^(NSDictionary *dic) {
            if (onResponse) {
                onResponse(nil);
            }
        }];
...
//绑定qq
- (void)bindingTencentWithDictionary:(NSDictionary *)dic OnResponse:(void (^)(NSDictionary *dic))onResponse{
    
    [YKHttpClient Method:@"POST" URLString:BingTencent_Url paramers:dic success:^(NSDictionary *dict) {
        [LBProgressHUD hideAllHUDsForView:[UIApplication sharedApplication].keyWindow animated:YES];
        if ([dict[@"status"] intValue] == 200) {
            
            [smartHUD alertText:[UIApplication sharedApplication].keyWindow alert:@"qq绑定成功" delay:1.2];
...
//解除微信QQ绑定
- (void)exitWeChatByTencentType:(NSInteger )type OnResponse:(void (^)(NSDictionary *dic))onResponse{
    ...
    NSString *url = [NSString stringWithFormat:@"%@?type=%@",exitBing_Url,@(type)];
