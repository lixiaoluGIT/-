...
//    btn.layer.borderWidth = 1;
    ...
    la.textColor = [UIColor colorWithHexString:@"ffffff"];
    la.font = PingFangSC_Semibold(kSuitLength_H(20));
    [header addSubview:la];
    //正在使用文字
    UILabel *la1 = [[UILabel alloc]initWithFrame:CGRectMake(kSuitLength_H(46), la.bottom + kSuitLength_H(8), kSuitLength_H(200), kSuitLength_H(18))];
    la1.textColor = [UIColor colorWithHexString:@"ffffff"];
    ...
    if (depositStatus == 1) {//使用中
        ...
        //判断卡类型
        if (cardType==2) {//季卡
            image.image = [UIImage imageNamed:@"季卡正在使用"];
            la.text = @"尊享季卡会员";
        ...
        if (cardType==3) {//年卡
            image.image = [UIImage imageNamed:@"年卡正在使用"];
             la.text = @"尊享年卡会员";
        }
        if (cardType==7) {//兑换卡
...
//    label.frame = CGRectMake(0, image.frame.size.height+image.frame.origin.y+20, WIDHT, 10);
    ...
    line.frame = CGRectMake(0, image.bottom+kSuitLength_H(18), WIDHT, kSuitLength_H(10));
    [header addSubview:line];
    
    UIImageView *image2 = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"Group-2"]];
    CGFloat scale2 = image2.frame.size.width/image.frame.size.height;
    image2.frame = CGRectMake(30, line.bottom+kSuitLength_H(21), WIDHT-60, (WIDHT-60)/scale2+kSuitLength_H(35));
    if (WIDHT==320) {
        image2.frame = CGRectMake(30, line.bottom+kSuitLength_H(21), WIDHT-60, (WIDHT-60)/scale2+50);
    }
